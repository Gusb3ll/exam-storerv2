    ...
    <div class="mx-auto">
      ...
      <div class="mt-8 space-y-6">
        <div class="-space-y-px rounded-md shadow-sm">
          ...
          <div class="pt-4">
            <nuxt-link to="/exam/subj/soc/add">
              <button
                class="add-button"
                :class="{
                  'transition-all opacity-80 pointer-events-none': loading
...
export default {
  ...
  methods: {
    async handleSearch () {
      this.loading = true
      const { data } = await this.$supabase.from('soc').select('ans').range(0, 0).eq('key', this.search)
      this.result = data
      this.loading = false
    },
    async FetchDB () {
      this.loading = true
      const { data } = await this.$supabase.from('soc').select('*')
      this.exam = data
      this.loading = false
    }
  }
